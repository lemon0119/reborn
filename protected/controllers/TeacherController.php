...
class TeacherController extends CController {
    ...
    public function actionEditLook(){
           ...
           public function actionEditLookInfo(){
            ...
            $thisLook -> update();
            if(Yii::app()->session['lastUrl'] == "modifyWork")
            {
                $Info = $this->getSuiteInfo("look", Yii::app()->session['suiteID']);           
                $this->render("ModifyEditLook",array(
                    'type' => "look",
                    'suite' => $Info['suite'],
                    'currentLesson' =>$Info['lesson'],
                    'currentClass' => $Info['class'],
                    'exerciseID'      => $exerciseID,
    ...
    public function actionEditKey(){
         ...
         public function actionEditKeyInfo(){
           ...
           if(Yii::app()->session['lastUrl'] == "modifyWork")
            {
                $Info = $this->getSuiteInfo("key", Yii::app()->session['suiteID']);           
 ...
 public function actionAddListen(){
     ...
     {                  
            ...
            if(Yii::app()->session['lastUrl'] == "modifyWork")
            {
                $Info = $this->getSuiteInfo("listen", Yii::app()->session['suiteID']);           
   ...
   //宋杰 2015-7-30 点击查看/编辑按钮
             ...
             public function actionEditChoiceInfo()
	{
            ...
            if(Yii::app()->session['lastUrl'] == "modifyWork")
            {
                $Info = $this->getSuiteInfo("choice", Yii::app()->session['suiteID']);           
   ...
   //查找简单题
    ...
    public function actionReturnFromAddQuestion()
     ...
     public function ModifyEditChoice(){
            ...
            $result     =   Yii::app()->db->createCommand($sql)->query();
            $result     =   $result->read();
            $Info = $this->getSuiteInfo($type, $suiteID);           
                $this->render("ModifyEditChoice",array(
                    'type' => $type,
                    'suite' => $Info['suite'],
                    'currentLesson' =>$Info['lesson'],
     ...
     public function ActionStuWork()
     {
         ...
         foreach ( $result['suiteLst'] as $suite)
             array_push($array_suiteLessonClass,$suite);
         $pages = $result['pages'];
         $result = Suite::model()->getSuiteByClassLessonSuite($teacherID);
         foreach ($result as $suite)
             array_push ($array_suite, $suite);
         if(!empty($teacher_class))
         {
           if(isset($_GET['classID']))
             Yii::app()->session['currentClass'] = $_GET['classID'];
           else
               Yii::app()->session['currentClass'] = $array_suiteLessonClass[0]['classID'];
           ...
           foreach ($teacher_class as $class)
             ...
             if(!empty($array_lesson))
             {
                if(isset($_GET['lessonID']))          
                   ...
                   Yii::app()->session['currentLesson'] = $array_suiteLessonClass[0]['lessonID'];
         ...
         $suiteID = $array_suiteLessonClass[0]['suiteID'];
         $class_student = Student::model()->findAll("classID = '$currentClass'");
         foreach ( $class_student as $student)
         {
             $result = SuiteRecord::model()->getRecordBySuiteAndStudentID($suiteID,$student['userID'],$currentClass,$currentLesson);
     ...
     public function ActionChangeSuiteClass()
     {
         ...
         {
             //从表中删除
             ClassLessonSuite::model()->deleteAll("classID=? and lessonID=? and suiteID=?" ,array($currentClass,$currentLesson,$suiteID));          
         }else
         {
     ...
     //局部刷新
     public function ActionPartFreshPage(){
