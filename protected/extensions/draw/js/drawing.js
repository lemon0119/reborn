...
DrawingPad = function(options) { 
	...
	function createNewClient(data){
		...
		if(DP.thisObj.id === data.listenerId && !DP.thisObj[data.senderId]){ //test to see if this instance is the one i want.
			if(confirm(data.senderName + "分享白板")){
	...
	*/
	function setConfirmShare(data){
		...
		if(DP.thisObj.id === data.senderId){
			if(data.isSharing){
				message = data.senderName + " 同意分享";
				//create new canvas
				DP.isSharing = true;
				_createSharedCanvas(data.listenerId);
			} else {
				message = data.senderName + " 不同意分享";
			}
			alert(message);
		}
	}
	...
	function _setEventHandlers(){
		...
		$(".toolbar").find(".sprite").click(function(){
			...
			if($(this).hasClass(tools[0])){			//share
				...
				'<h4>注意!</h4>' +
				'<p>是否确定清空图片</p><br/>' +
				'<a class="btn btn-danger" href="#" data-dismiss="alert">清空</a> <a class="btn btn-default" href="#" data-dismiss="alert">NO don\'t!</a>' +
				'</div>');
	...
	this.init = function(selector) {
		...
		DP.thisObj.socket = io.connect("http://192.168.101.235:4000");
