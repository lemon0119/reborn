...
<!--                   <button id="close_exercise" class="fr btn btn-primary">关闭</button>-->
    ...
    <?php } ?>
        ...
        var originalContent = "<?php echo $exerOne['content']; ?>";
        window.GA_originalContent = originalContent;
    <?php
    $exerciseID = $exerOne['exerciseID'];
    $category = $exerOne['category'];
    $studentID = Yii::app()->session['userid_now'];
    $sqlClassExerciseRecord =  AnswerRecord::model()->findAll("exerciseID = '$exerciseID' AND recordID = '$recordID' AND type = 'key' AND category = '$category' AND createPerson='$studentID'");
    $countSquence = count($sqlClassExerciseRecord);
    $squence = $countSquence;
    ?>
        console.log('<?php echo $exerOne['exerciseID']; ?>');
        window.G_squence = <?php echo $squence; ?>;
        var recordID = <?php echo $recordID?>
        var classExerciseID = <?php echo $exerciseID; ?>;
        var type = "key";
        window.G_exerciseData = Array(classExerciseID, type,recordID);
