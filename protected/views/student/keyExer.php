    ...
    <?php } ?>
        ...
        <div id="span" class="hero-unit" align="center">
            <table style="width: 580px"  border = '0px'><button id="finish" onclick="finish()" style="margin-left:30px;" class="fl btn btn-primary" >完成练习</button><button id="close_exercise" class="fr btn btn-primary">关闭</button>
                <tr><h3><?php foreach ($exercise2['key'] as $value) {
            print_r($value['title']);
        }   ?></h3></tr>
                <tr>
                    ...
                    <td colspan="2"><span style="color:green" id="isDone">0</span>/<span  style="color: #f46500" id="AllOfWord"></span><span hidden="true" style="color: #f46500" id="repeatNum"><?php //echo $exercise['repeatNum'] ?></span ></td>
                <tr>
...
//                    require Yii::app()->basePath . "\\views\\student\\speed_keyboard.php";
        ...
        <form name='nm_answer_form' id='id_answer_form' method="post" action="<?php echo $host . $path . $page . $param; ?>">
            <input id="id_content" type="hidden" value="<?php //echo $exercise['content']; ?>">
            <input id="id_speed" type="hidden" value="<?php //echo $exercise['speed']; ?>">
            <input  name="nm_answer"id="id_answer" type="hidden">
            <input  name="nm_cost" id="id_cost" type="hidden">
        </form>
    </div>
                ...
                var isover = setInterval(function () {
                    ...
                    if (time == 0) {
                        ...
                        doSubmit(true, function () {
                            window.location.href = "index.php?r=student/examkeyType&&over=1&&exerID=<?php echo $exerID; ?>&&cent=<?php $arg = implode(',', $cent);
    echo $arg; ?>";
                        });
